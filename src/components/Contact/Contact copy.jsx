...
const Contact = () => {
  ...
  const handleUsernameFocus = () => {
    setFormErrors(validateUername(formValues));
  };
  const handleEmailFocus = () => {
    setFormErrors(validateEmail(formValues));
  };
  const handleMessageFocus = () => {
    setFormErrors(validateMessage(formValues));
  };
  let errors = {};
  const validateUername = (values) => {
    if (!values.username) {
    ...
    } else if (values.username.lenth < 5) {
      errors.username = "user name length is greater than 5 charaters ";
    }
